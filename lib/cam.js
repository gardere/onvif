...
Cam.prototype._request = function(options, callback) {
	...
	var req = http.request(reqOptions, function(res) {
		...
		res.on('end', function() {
			...
			_this.emit('rawResponse_ext', {...reqOptions, ...options, ...{xml} });
	...
	this.emit('rawRequest_ext', { ...reqOptions, ...options });
...
Cam.prototype.getActiveSources = function() {
	...
	/**
	 * Default profile for the device
	...
	var idx = 0;
	this.videoSources.forEach(function(videoSource, videoSourcesIdx) {
		...
		if (this.defaultProfiles[idx].videoEncoderConfiguration) {
			this.activeSources[idx].encoding = this.defaultProfiles[idx].videoEncoderConfiguration.encoding;
			this.activeSources[idx].width = this.defaultProfiles[idx].videoEncoderConfiguration.resolution.width;
			this.activeSources[idx].height = this.defaultProfiles[idx].videoEncoderConfiguration.resolution.height;
			this.activeSources[idx].fps = this.defaultProfiles[idx].videoEncoderConfiguration.rateControl.frameLimit;
			this.activeSources[idx].bitrate = this.defaultProfiles[idx].videoEncoderConfiguration.rateControl.bitrateLimit;
		}
		...
		if (idx === 0) {
			 ...
			 * @type {Cam~ActiveSource}
			 */
			this.activeSource = this.activeSources[0];
		}
